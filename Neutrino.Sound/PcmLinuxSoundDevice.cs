...
internal unsafe class PcmLinuxSoundDevice : PcmSoundDevice
{
    ...
    public override void Open(string name = "default")
    {
        ...
        fixed (Alsa.snd_pcm_t** handlePtr = &_handle)
        {
            int errorCode = _api.snd_pcm_open(handlePtr,
                ptr,
                Alsa.snd_pcm_stream_t.SND_PCM_STREAM_PLAYBACK, 0);
    ...
    public void WriteStream(Stream wavStream, WaveFile.WaveFileHeader header, nint hwParams, ref int dir)
    {
        ...
        try
        {
            ...
            fixed (byte* buffer = readBuffer)
            {
                ...
                {
                    errorCode = _api.snd_pcm_writei(_handle, buffer, frames);
                    Console.WriteLine(wavStream.Position);
                    AlsaException.ThrowOnError(errorCode);
                }
            }
    ...
    public void Initialize(WaveFile.WaveFileHeader header, ref nint hwParams, ref int dir)
    {
        ...
        errorCode = _api.snd_pcm_hw_params_set_access(_handle, 
            hwParams, 
            Alsa.snd_pcm_access_t.SND_PCM_ACCESS_RW_INTERLEAVED);
        AlsaException.ThrowOnError(errorCode);

        errorCode = (header.BitsPerSample / 8) switch
        {
            1 => _api.snd_pcm_hw_params_set_format(_handle, hwParams, Alsa.snd_pcm_format_t.SND_PCM_FORMAT_U8),
            2 => _api.snd_pcm_hw_params_set_format(_handle, hwParams, Alsa.snd_pcm_format_t.SND_PCM_FORMAT_S16_LE),
            3 => _api.snd_pcm_hw_params_set_format(_handle, hwParams, Alsa.snd_pcm_format_t.SND_PCM_FORMAT_S24_LE),
