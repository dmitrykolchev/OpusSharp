...
internal unsafe class Alsa : ApiTable
{
    public Alsa(nint module) : base(module)
    {
    }

    public static Alsa Load(string libraryName)
    {
        nint libraryHandle = NativeLibrary.Load(libraryName);
        return new Alsa(libraryHandle);
    }
    /**
     * \file include/pcm.h
    ...
    [Import]
    public delegate* unmanaged[Cdecl]
        <snd_pcm_t*, byte*, ulong, int> snd_pcm_writei;
