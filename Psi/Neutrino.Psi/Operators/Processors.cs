...
public static partial class Operators
{
    ...
    {
        Processor<TIn, TOut> select = new(source.Out.Pipeline, transform, name: name);
    ...
    {
        return Process<TIn, TOut>(source, (d, e, s) => s.Post(selector(d, e), e.OriginatingTime), deliveryPolicy, name);
    }
    ...
    {
        return Select(source, (d, e) => selector(d), deliveryPolicy, name);
    }
    ...
    public static IProducer<TOut?> NullableSelect<TIn, TOut>(this IProducer<TIn?> source, Func<TIn, Envelope, TOut> selector, DeliveryPolicy<TIn?> deliveryPolicy = null, string name = nameof(NullableSelect))
        ...
        where TOut : struct
    {
        return source.Select((v, e) => v.HasValue ? new TOut?(selector(v.Value, e)) : null, deliveryPolicy, name);
    }
    ...
    public static IProducer<TOut?> NullableSelect<TIn, TOut>(this IProducer<TIn?> source, Func<TIn, TOut> selector, DeliveryPolicy<TIn?> deliveryPolicy = null, string name = nameof(NullableSelect))
        ...
        where TOut : struct
    {
        return source.Select(v => v.HasValue ? new TOut?(selector(v.Value)) : null, deliveryPolicy, name);
    }
    ...
    {
        return source.Select(t => t.Item1, deliveryPolicy, name);
    }
    ...
    {
        return source.Select(t => t.Item2, deliveryPolicy, name);
    }
    ...
    {
        return Do(source, (d, e) => action(d), deliveryPolicy, name);
    }
    ...
    {
        IProducer<(bool original, bool upsert, T m)> originalStream = source.Select(m => (original: true, upsert: true, m), DeliveryPolicy.Unlimited);
        IEnumerable<((bool original, bool upsert, T message), DateTime originatingTime)> orderedEdits = edits.OrderBy(e => e.originatingTime).Select(e => ((original: false, e.upsert, e.message), e.originatingTime));
        IProducer<(bool original, bool upsert, T message)> editsStream = Generators.Sequence(source.Out.Pipeline, orderedEdits);
        return originalStream
            ...
            .Process<(bool original, bool upsert, T message)[], T>(
                ...
                {
                    (bool original, bool upsert, T message) = messages[0].original && messages.Length > 1 ? messages[1] : messages[0];
                    if (upsert)
                    {
