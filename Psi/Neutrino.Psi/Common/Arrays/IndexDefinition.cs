// <copyright file="IndexDefinition.cs">
...
public abstract class IndexDefinition
{
    ...
    /// <returns>An index definition for the specified range.</returns>
    public IndexDefinition Slice(int start, int end) => Slice(new Range(start, end));
    ...
    {
        if (ElementStride != other.ElementStride)
        {
            throw new InvalidOperationException("The index definitions are incompatible.");
        }

        return new DiscreteIndexDefinition(_elementStride, Values.Concat(other.Values).ToArray());
    }
