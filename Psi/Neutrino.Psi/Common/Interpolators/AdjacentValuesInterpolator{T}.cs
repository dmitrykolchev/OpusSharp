// <copyright file="AdjacentValuesInterpolator{T}.cs">
...
{
    private readonly AdjacentValuesInterpolator<T, T> _interpolator;
    ...
    public AdjacentValuesInterpolator(Func<T, T, double, T> interpolatorFunc, bool orDefault, T defaultValue = default, TimeSpan? maxSpan = null, string name = null)
    {
        ...
        _interpolator = new(InterpolateWithEndpointHandling, orDefault, defaultValue, maxSpan, name);
    }

    /// <inheritdoc/>
    public override InterpolationResult<T> Interpolate(
        DateTime interpolationTime, 
        IEnumerable<Message<T>> messages, 
        DateTime? closedOriginatingTime)
    {
        return _interpolator.Interpolate(interpolationTime, messages, closedOriginatingTime);
    }

    /// <inheritdoc/>
    public override string ToString()
    {
        return _interpolator.ToString();
    }
}
