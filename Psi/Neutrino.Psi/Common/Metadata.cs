...
public abstract class Metadata
{
    ...
    internal static Metadata Deserialize(BufferReader metadataBuffer)
    {
        ...
        string name = metadataBuffer.ReadString();
        int id = metadataBuffer.ReadInt32();
        string typeName = metadataBuffer.ReadString();
        int version = metadataBuffer.ReadInt32();
        string serializerTypeName = metadataBuffer.ReadString();
        int serializationSystemVersion = metadataBuffer.ReadInt32();
        ushort customFlags = metadataBuffer.ReadUInt16();
        MetadataKind kind = (MetadataKind)metadataBuffer.ReadUInt16();

        if (kind == MetadataKind.StreamMetadata)
        {
            PsiStreamMetadata result = new(name, id, typeName, version, serializationSystemVersion, customFlags);
            result.Deserialize(metadataBuffer);
            return result;
        }
        else if (kind == MetadataKind.TypeSchema)
        {
            TypeSchema result = new(typeName, name, id, version, serializerTypeName, serializationSystemVersion);
            result.Deserialize(metadataBuffer);
            return result;
        }
        else if (kind == MetadataKind.RuntimeInfo)
        {
            RuntimeInfo result = new(name, version, serializationSystemVersion);
            return result;
        }
        else
        {
