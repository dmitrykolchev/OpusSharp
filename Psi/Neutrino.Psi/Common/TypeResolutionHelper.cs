...
/// </summary>
public static partial class TypeResolutionHelper
{
    ...
    internal static string RemoveAssemblyName(string assemblyQualifiedName)
    {
        ...
        typeName = RemoveAssemplyNameExpression().Replace(typeName, string.Empty);
    ...
    internal static string RemoveCoreAssemblyName(string assemblyQualifiedName)
    {
        ...
        typeName = RemoveCoreAssemblyNameExpression().Replace(typeName, string.Empty);
    ...
    private static Assembly AssemblyResolver(AssemblyName assemblyName)
    {
        ...
        Assembly assembly = loadedAssemblies.FirstOrDefault(a => a.GetName().FullName == assemblyName.FullName);
        if (assembly != null)
        {
            return assembly;
        }
    ...
    [GeneratedRegex(@",\s[^,\[\]\*]+")]
    private static partial Regex RemoveAssemplyNameExpression();

    [GeneratedRegex(@",\s(mscorlib|System\.Private\.CoreLib)[^\[\]\*]*")]
    private static partial Regex RemoveCoreAssemblyNameExpression();
}
