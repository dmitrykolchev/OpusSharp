...
public class WaveFormat
{
    ...
    public static WaveFormat MarshalFromPtr(IntPtr ptr)
    {
        ...
        if (ptr != IntPtr.Zero)
        {
            ...
            if (waveFormat.ExtraSize > 0)
            {
                WaveFormatEx waveFormatEx = new();
                waveFormatEx.CopyFrom(waveFormat);
                IntPtr extraInfoPtr = new(ptr.ToInt64() + Marshal.SizeOf<WaveFormat>());
    ...
    {
        MemoryStream formatStream = new();
        using BinaryWriter writer = new(formatStream);
        writer.Write(this);
    ...
    public override bool Equals(object other)
    {
        ...
        // field comparison
        return (FormatTag == otherFormat.FormatTag) &&
            (Channels == otherFormat.Channels) &&
            (SamplesPerSec == otherFormat.SamplesPerSec) &&
            (AvgBytesPerSec == otherFormat.AvgBytesPerSec) &&
            (BlockAlign == otherFormat.BlockAlign) &&
            (BitsPerSample == otherFormat.BitsPerSample) &&
            (ExtraSize == otherFormat.ExtraSize);
    }
    ...
    public override int GetHashCode()
    {
        ...
        int hash = 17;
        hash = (hash * 23) + FormatTag.GetHashCode();
        hash = (hash * 23) + Channels.GetHashCode();
        hash = (hash * 23) + SamplesPerSec.GetHashCode();
        hash = (hash * 23) + AvgBytesPerSec.GetHashCode();
        hash = (hash * 23) + BlockAlign.GetHashCode();
        hash = (hash * 23) + BitsPerSample.GetHashCode();
        hash = (hash * 23) + ExtraSize.GetHashCode();
        return hash;
    }
}
