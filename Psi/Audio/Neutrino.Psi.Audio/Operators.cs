...
public static class Operators
{
    ...
    {
        return source.PipeTo(new Reframe(source.Out.Pipeline, frameSizeInBytes, name), deliveryPolicy);
    }
    ...
    {
        return source.PipeTo(new Reframe(source.Out.Pipeline, frameDuration, name), deliveryPolicy);
    }
    ...
    {
        return source.Select(x => x.SelectChannel(channel), deliveryPolicy, name);
    }
    ...
    {
        return source.Select(x => x.Data, deliveryPolicy, name);
    }
    ...
    {
        return source.Select(x => new AudioBuffer(x, audioFormat), deliveryPolicy, name);
    }
    ...
    {
        return source.PipeTo(new FrameShift(source.Out.Pipeline, frameSizeInBytes, frameShiftInBytes, bytesPerSec, name), deliveryPolicy);
    }
    ...
    {
        return source.PipeTo(new ToFloat(source.Out.Pipeline, format, name), deliveryPolicy);
    }
    ...
    public static IProducer<float[]> Dither(this IProducer<float[]> source, float scaleFactor, int randomSeed = 0, DeliveryPolicy<float[]> deliveryPolicy = null, string name = nameof(Dither))
    {
        ...
        return source.Select(
            values =>
            {
                ...
                {
                    dithered[i] = values[i] + (((random.Next() / (float)int.MaxValue * 2.0f) - 1.0f) * scaleFactor);
                }
    ...
    {
        return source.Select<float[], float[]>(new HanningWindow(kernelLength).Apply, deliveryPolicy, name);
    }
    ...
    {
        return source.PipeTo(new FFT(source.Out.Pipeline, fftSize, inputSize, name), deliveryPolicy);
    }
    ...
    {
        return source.PipeTo(new FFTPower(source.Out.Pipeline, name), deliveryPolicy);
    }
    ...
    {
        return source.PipeTo(new LogEnergy(source.Out.Pipeline, name), deliveryPolicy);
    }
    ...
    {
        return source.PipeTo(new ZeroCrossingRate(source.Out.Pipeline, name), deliveryPolicy);
    }
    ...
    {
        return source.PipeTo(new FrequencyDomainEnergy(source.Out.Pipeline, start, end, name), deliveryPolicy);
    }
    ...
    {
        return source.PipeTo(new SpectralEntropy(source.Out.Pipeline, start, end, name), deliveryPolicy);
    }
        ...
        string name = nameof(Streamline))
    {
        ...
        {
            DateTime unpleatedAudioTime = DateTime.MinValue;
            return source
                ...
                .Process<IEnumerable<Message<AudioBuffer>>, AudioBuffer>(
                    ...
                    {
                        Message<AudioBuffer> last = messages.LastOrDefault();
        ...
        else if (audioStreamlineMethod == AudioStreamlineMethod.Pleat)
        {
            ...
            return source.Window(
                ...
                messages =>
                {
                    Message<AudioBuffer> last = messages.LastOrDefault();
                    Message<AudioBuffer> first = messages.FirstOrDefault();
                    double delta = (last.OriginatingTime - last.Data.Duration - first.OriginatingTime).TotalSeconds;
                    if (firstPleat)
                    {
                        firstPleat = false;
                        AudioBuffer newBuffer = new(last.OriginatingTime - first.OriginatingTime + first.Data.Duration, last.Data.Format);
                    ...
                    else
                    {
                        if (delta > 0)
                        {
                            AudioBuffer newBuffer = new(last.OriginatingTime - first.OriginatingTime, last.Data.Format);
                            Array.Copy(last.Data.Data, 0, newBuffer.Data, newBuffer.Data.Length - last.Data.Length, last.Data.Length);
                            return newBuffer;
                        }
                        else if (delta < 0)
            ...
            return source
                ...
                .Process<IEnumerable<Message<AudioBuffer>>, AudioBuffer>(
                    (messages, envelope, emitter) =>
                    {
                        ...
                        TimeSpan offset = last.OriginatingTime - unpleatedAudioTime;
