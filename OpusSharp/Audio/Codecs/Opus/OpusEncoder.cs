...
public class OpusEncoder : IDisposable
{
    ...
    {
        IntPtr encoder = OpusApi.opus_encoder_create((int)samplingRate, (int)channels, (int)application, out int error);
    ...
    public unsafe int Encode(short[] pcm, int offset, int frameSize, byte[] data)
    {
        ...
        {
            int length = OpusApi.opus_encode(_encoder, new IntPtr(pcmPtr), frameSize, new IntPtr(dataPtr), data.Length);
            if (length < 0)
            {
    ...
    public void SetComplexity(int level)
    {
        ...
        int errorCode = OpusApi.opus_encoder_ctl(_encoder, OpusApi.OPUS_SET_COMPLEXITY_REQUEST, level);
    ...
    {
        int errorCode = OpusApi.opus_encoder_ctl(_encoder, OpusApi.OPUS_SET_BITRATE_REQUEST, (int)value);
        OpusException.HandleError(errorCode);
    }
    /// <summary>
    /// Configures the bitrate in the encoder.
    ...
    {
        int errorCode = OpusApi.opus_encoder_ctl(_encoder, OpusApi.OPUS_SET_BITRATE_REQUEST, value);
        OpusException.HandleError(errorCode);
    }
